...
class Rule(RuleFactory):
    ...
    `websocket`
        If ``True``, this rule is only matches for WebSocket requests.
        For security, WebSocket Secure (``wss://``) should always be used
        instead of insecure ``ws://`` connections. By default, rules will
